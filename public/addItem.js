...
document.querySelector("#addItemBtn").addEventListener("click", ()=>{
    ...
    input.innerHTML = `Item name: <input type="text" name="item_name${x}" autocomplete="off"/> <br/><br/>Price: <input type="text" name="price${x}" autocomplete="off"/><br/><br/>Description: <input type="text" name="product_desc${x}" autocomplete="off"/><br/><br/>Image URL: <input type="text" name="imgURL${x}" autocomplete="off"/><br/><br/><input type="hidden" name="item_id${x}"><button type="button" class="removeItemBtn" id="remove${x}">Delete Item</button><br/><br/>`
